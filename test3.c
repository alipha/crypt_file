...
int main(void) {
    ...
    if((status = crypt_seek(cf, 7995, SEEK_SET))) {
        fprintf(stderr, "crypt_seek error: %s\n", crypt_error(status));
        return 1;
    }

    if((status = crypt_write(cf, "ABCDEFGHIJKLMNOPQRSTUVWXYZ", 26))) {
        fprintf(stderr, "crypt_write error: %s\n", crypt_error(status));
        return 1;
    }
    ...
    if((status = crypt_seek(cf2, 5, SEEK_SET))) {
        fprintf(stderr, "crypt_seek error: %s\n", crypt_error(status));
        return 1;
    }

    if((status = crypt_read(cf2, buffer, 10, &amount_read))) {
        fprintf(stderr, "crypt_read error: %s\n", crypt_error(status));
        return 1;
    }
